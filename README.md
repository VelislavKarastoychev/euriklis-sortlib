...
- In version 1.1.7 the error message corresponded to the non number array input in the bucket_sort method was correctly declared and connected with the main method (The file corresponded to this error was not declared in the index file of the errors).In the same method the setting of the buckets was precessed. In the implementation of the sort_object_array was made changes for the selection of the bucket sort algorithm (when the algorithm was selected, the buckets was not set). Also the method add_element_in_sorted_array was changed in more correct and time efficient form. In addition a set of new methods of the addition family was created, such like the *SortLib.add_element_in_sorted_object_array_by_property(array, property, element)*, *find_elements_in_sorted_array(array, element)*, *find_elements_in_sorted_object_array_by_property(array, property, element)* and *filter_with_validator(array, callback)*. The jsdoc (commentaries and documentation protocol) of the static method generate_random_array was changed and the method was expanded with such a way in which in the callback function to be inserted and the indices of the elements. Also a static method generate_random_string_array() was created. Changes was made also into the jsdoc of the insertion sort method.
