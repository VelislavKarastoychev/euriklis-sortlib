...
- In version 1.1.6 optimization techniques was applied to the method find_worst_for_object_array_by_property. Corrections in the method find_worst_elements for the warnings messages. Also correction in the warning messages in the find_best_elements was done. It is possible some methods of the version to be problematic, because of the @euriklis/validator@1.2.0 and @euriklis/message@1.2.0 versions. So we recommend you to use the version 1.1.6. The methods of the 1.1.5 are stable but the versions of the euriklis/validator and euriklis/message for versions 1.2.x are in the so called esm mode.
- In version 1.1.7 a number of new methods were added, some of the existing methods were extended functionally, and in some places the implementation itself was corrected or optimized. Namely we add the methods of so called searching family like the *SortLib.find_elements_in_sorted_array(array, element)* (returns {array: array of items which are equals to the element, indices: array of numbers which are corresponded to the indices of the items, equals to the element}), *SortLib.find_elements_in_sorted_object_array_by_property(array, property, element)*, which returns an object with structure {array: array of object items which are equal to the element}(in this method the element is object with keys, the string elements of the property array or if the property is string an object with key the property value) and finally the similar to them method *SortLib.filter_with_validator(array, callback)*, where the callback is a function with arguments a validator instance and a number, representing the index of the element in the array. Also methods for adding of elements in sorted array was added, such like the method *SortLib.add_element_in_sorted_object_array_by_property(array, property, element)* which is similar to the add_element_in_sorted_array(array, element). Note that the method *SortLib.add_element_in_sorted_array(array, element)* was changed in order to be optimized and **corrected** (errors in the case in which the mode was descending order was detected and also the testing of the type consistency of the method arguments was non efficient). In order to achieve  completeness, we also created methods for removing of a element from a sorted array and also a method that creates an array with random strings (in addition to the method generate_random_array(n, seed, callback)). Also from version 1.1.7, this method extended in such a way in which the callback parameter (a function) may takes two parameters, that are namely the element and the index of the random element in the generated array. 
In this version (1.1.7) a set of changes in the error and warnings messages was revised and changed. Thus the error message corresponded to the non number array input in the bucket_sort method was correctly declared and connected with the main method (The file corresponded to this error was not declared in the index file of the errors).In the same method the setting of the buckets was precessed. In the implementation of the sort_object_array was made changes for the selection of the bucket sort algorithm (when the algorithm was selected, the buckets was not set). Also the method add_element_in_sorted_array was changed in more correct and time efficient form. The warning message of the method find_best_elements was revised in order to be shown when the user does not insert an integer number in closed range consisted with the length of the array of a floating point number in the open interval (0, 1). The jsdoc (commentaries and documentation protocol) of the static method generate_random_array was changed and the method was expanded with such a way in which in the callback function to be inserted and the indices of the elements. Also a static method generate_random_string_array() was created. Changes was made also into the jsdoc of the insertion sort method.
In version 1.1.8 the methods add_element_to_sorted_array and the other derivative method was updated in such a way in which when the array is empty the element to be added without testings and throwing of error messages.
In version 1.1.9 the method find_element_in_sorted_object_array_by_property() and the method remove_element_from_sorted_/object_array() was set to return empty output when the array is empty without additional tests and throwing of error messages. In version 1.1.10 small reconfiguration was made. In version 1.1.11 corrections in the find_elements_in_sorted_array and find_elements_in_sorted_object_array_by_property which was significant were made.
